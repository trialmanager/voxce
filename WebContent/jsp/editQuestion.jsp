...
<script type="text/javascript">
	var questionId = "<c:out value = '${questionHtml.question_id}' />";
	ddaccordion.init({
	...
	function back() {
		window.location.href = "ShowCRFIndex.crf?CurrentForm=list_question_form";

	}
	$(document).ready(
			function() {
				$(".questions-container").find(".select-answer-type").find(
				...
				$(".questions-container").find(".select-answer-type").find(
						...
						function() {
							...
							if ($(this).attr("value") == $(
									".questions-container").find(
									".each-ques-crf").attr("anstype")) {
				...
				<div class="body_inner" id="add_question_form"
					...
					<div class="body_inner_header">
						...
						<div class="crumb_mid">
							...
							<div class="crumbs-link">
								<a
									href="<c:url value='ShowCRFIndex.crf?CurrentForm=list_question_form' />">Question
									List</a>
							</div>
							<div class="crumbs-link">
					...
					<div class="create_group_container">
						<div id="" class="myform library">
							...
							<form id="formAddQuestion" name="formAddQuestion" method="POST"
								action="editQuestion.crf" class="library">

								<span id="AddQTGreenMessage" class="chk_avail_yes"
									style="display: none;">${message}</span> <span
									id="AddQTErrorMessage" class="error_label"
									style="display: none;">${error_message}</span>


								<div class="questions-container"
									style="border: 1px solid scrollbar; width: 95%; float: left; padding: 5px; margin: 5px;">
									${questionHtml.question_edit_html}</div>
								<div class="hiddenQuestion" style="display: none;">
									${questionHtml.question_edit_html}</div>
								...
								<div style="clear: both;">
									<input type="hidden" id="htmlData" name="htmlData"> <input
										...
										type="hidden" value="${questionHtml.question_id}"
										id="editHtmlid" name="questionId"> <input
										type="hidden" value="" id="change" name="change"> <input
										type="hidden" value="" id="oldQuestionName"
										name="oldQuestionName" />
									<button id="submit_question" edit="edit" type="button"
										style="width: 100px; margin-left: 39%; height: 34px;"
										class="blue-pill save-question">Save</button>
									<input id="cancleSubmit" onclick="back()" type="button"
										style="width: 100px;" class="grey-pill" value="Cancel">
								</div>
								...
								<form id="view_question_form">
									...
									<table width="60%" border="0" cellspacing="1" cellpadding="0"
										...
										<tr class="rights_header_view_list">
											...
											<c:if
												test="${tl:rightsofView(resourceList,'Edit Questions')==1 }">
												<td width="12%" align="left" valign="middle">&nbsp;Edit</td>
											</c:if>
											<c:if
												test="${tl:rightsofView(resourceList,'Delete Questions')==1 }">
												...
												<tr class="${tempRowStyle}" id="${questions.question_id}">
													...
													<c:if
														...
														<td valign="top" align="left" class="set_padding" edit=''><a
															class="edit-crf-question" href="javascript://"><img
