	...
	<div class="wrapper">
		...
		<div class="body_content">
			...
			<div class="left_nav" id="MainDiv">
				...
				<div class="nav_body">
					<div class="nav_menu">
						<c:if test="${tl:rightsofView(resourceList,'Discrepancies')==1}">
							<a href="javascript://" class="table_disc"
								style="display: none; text-decoration: none; color: #484848; font-family: arial;"
								name="" onclick="" id="">Discrepancies</a>
						</c:if>
					</div>
				</div>
							...
							<div class="field_txt_view_list">
								...
								<table id="View_user_Table" width="100%" border="0"
									...
									<tr class="rights_header_view_list">
										...
										<td width="8%" align="left" valign="middle">&nbsp;Filled
											date</td>
										<td width="8%" align="left" valign="middle">&nbsp;Delete</td>
									</tr>
									<tbody id="fbodyo">
												...
												<c:forEach items="${patentCrf}" var="crf"
													...
													<c:if test="${crf.crf_id == mapeCrf.crf_id }">
														<td crfid="${mapeCrf.patient_crf_id}" valign="top"
															...
															htmlData='${mapeCrf.crf_html}'>${crf.crf_name}</a></td>
													</c:if>
														...
														<c:choose>
															<c:when test="${users.user_id == mapeCrf.filld_by}">
																<span username="${mapeCrf.filld_by}"
																	style="display: none;" class="name_user">${users.first_name}
											...
											<c:forEach items="${users}" var="users"
												...
												<c:choose>
													<c:when test="${users.user_id == mapeCrf1.modifiedBy}">
														<span username="${mapeCrf1.modifiedBy}" class="name_user">${users.first_name}
															${users.last_name} </span>
													</c:when>
												</c:choose>
											</c:forEach>
